...
void point3D::setWidth(double _w){
w=_w;
}
...
point3D point3D::projection(point3D _p_hat){
	...
	y_p=C*_p_hat.y;
	z_p=C*_p_hat.z;
	return point3D(x_p,y_p,z_p,_p_hat.w);
}
...
string point3D::point2String(){
	return to_string(index)+" "+to_string(x)+" "+ to_string(y) + " "+ to_string(z)+ " "+ to_string(w);
}
...
//void point3D::operator-(const point3D& _p){
//	/*doesn't affect the point radius*/
//	this->x=-this->x-_p.x;
//	this->y=this->y-_p.y;
//	this->z=this->z-_p.z;
//}
double point3D::Distance(point3D &_p){
	point3D P;
	P.set(this->x-_p.x,this->y-_p.y,this->z-_p.z);
	return P.norm();
}
