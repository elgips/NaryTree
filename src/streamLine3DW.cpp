...
streamLine3D::streamLine3D(){
	point3D PI(0,0,0,1),X(1,0,0,1),Y(0,1,0,1),Z(0,0,1,1);
...
streamLine3D::streamLine3D(double _w){
	point3D PI(0,0,0,_w),X(1,0,0,1),Y(0,1,0,1),Z(0,0,1,1);
	point3D *PI1=&PI,*PE1=0x0;
	pH=X;
	pL=Y;
	pU=Z;
...
//}
streamLine3D::streamLine3D(point3D* _pi){
	point3D X(1,0,0,1),Y(0,1,0,1),Z(0,0,1,1);
	point3D *PE1=0x0;
	pH=X;
	pL=Y;
	pU=Z;
...
streamLine3D::streamLine3D(point3D *_pi,point3D _pH0,point3D _pL0,point3D _pU0){
	point3D X(1,0,0,1),Y(0,1,0,1),Z(0,0,1,1),PH(_pH0),PL(_pL0),PU(_pU0);
	point3D *PE1=0x0;
	vector<point3D> vP;
	PH.normalize();
	pH=PH;
...
streamLine3D::streamLine3D(point3D *_pi,point3D _pH0){
	point3D X(1,0,0,1),Y(0,1,0,1),Z(0,0,1,1),PH(_pH0),PL(Y),PU(Z);
	point3D *PE1=0x0;
	vector<point3D> vP;
	PH.normalize();
	pH=PH;
...
streamLine3D::streamLine3D(point3D *_pi,point3D *_pe){
	pI=_pi;
	pE=_pe;
	point3D X(1,0,0,1),Y(0,1,0,1),Z(0,0,1,1),PU(_pe->x-_pi->x,_pe->y-_pi->y,_pe->z-_pi->z,1),PL(Y),PH(X);
	vector<point3D> vP;
	PU.normalize();
	pU=PU;
	vP.push_back(PU);
...
// point3D pHT(this->pH.x,this->pH.y,this->pH.z);
 point3D pLT(cos(theta)*this->pL.x+sin(theta)*this->pU.x,cos(theta)*this->pL.y+sin(theta)*this->pU.y,cos(theta)*this->pL.z+sin(theta)*this->pU.z,1);
 point3D pUT(-sin(theta)*this->pL.x+cos(theta)*this->pU.x,-sin(theta)*this->pL.y+cos(theta)*this->pU.y,-sin(theta)*this->pL.z+cos(theta)*this->pU.z,1);
//this->pH=pHT;
this->pL=pLT;
this->pU=pUT;
}
...
// point3D pLT(this->pL.x,this->pL.y,this->pL.z);
 point3D pHT(cos(theta)*this->pH.x+sin(theta)*this->pU.x,cos(theta)*this->pH.y+sin(theta)*this->pU.y,cos(theta)*this->pH.z+sin(theta)*this->pU.z,1);
 point3D pUT(-sin(theta)*this->pH.x+cos(theta)*this->pU.x,-sin(theta)*this->pH.y+cos(theta)*this->pU.y,-sin(theta)*this->pH.z+cos(theta)*this->pU.z,1);
this->pH=pHT;
//this->pL=pLT;
this->pU=pUT;
}
...
// point3D pUT(this->pU.x,this->pU.y,this->pU.z);
 point3D pHT(cos(theta)*this->pH.x-sin(theta)*this->pL.x,cos(theta)*this->pH.y-sin(theta)*this->pL.y,cos(theta)*this->pH.z-sin(theta)*this->pL.z,1);
 point3D pUT(sin(theta)*this->pH.x+cos(theta)*this->pL.x,sin(theta)*this->pH.y+cos(theta)*this->pL.y,sin(theta)*this->pH.z+cos(theta)*this->pL.z,1);
this->pH=pHT;
//this->pL=pLT;
this->pU=pUT;
}
